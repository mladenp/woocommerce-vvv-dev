...
PHP
  ...
  wp core install --url=local.woocommerce.local --title='WooCommerce' --admin_user=admin --admin_password=password --admin_email=changme@changeme.com --allow-root
  ...
  wp menu item add-custom primary-menu Home http://local.woocommerce.local/ --allow-root
  ...
  wp search-replace 'wpthemetestdata.wordpress.com' 'local.woocommerce.local' --skip-columns=guid --allow-root
